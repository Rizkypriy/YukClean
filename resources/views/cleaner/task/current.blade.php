        ...
        <div class="bg-white rounded-xl border border-gray-200 p-5 shadow-sm mb-6">
            ...
            <div class="flex items-center gap-4 mb-4">
                <div class="w-14 h-14 bg-blue-100 rounded-full flex items-center justify-center">
                    <span class="text-blue-600 font-bold text-xl">{{ $currentTask->cleaner->initials ?? 'CS' }}</span>
                </div>
                <div>
                    <h3 class="font-bold text-lg text-gray-800">{{ $currentTask->customer_name }}</h3>
                    <p class="text-sm text-gray-600">{{ $currentTask->service_name }}</p>
                </div>
            </div>

            <div class="space-y-3 text-sm mb-4">
                <div class="flex items-start gap-3">
                    <i class="fas fa-location-dot text-gray-400 w-5 mt-0.5"></i>
                    <span class="text-gray-700">{{ $currentTask->address }}</span>
                </div>
                <div class="flex items-start gap-3">
                    <i class="far fa-calendar text-gray-400 w-5 mt-0.5"></i>
                    <span class="text-gray-700">{{ $currentTask->formatted_date }}, {{ $currentTask->formatted_time }}</span>
                </div>
            </div>
            ...
            <div class="space-y-4 mt-6">
                ...
                <div class="flex items-start gap-3">
                    ...
                    <div class="flex-1">
                        ...
                        <button onclick="updateStatus('{{ $key == 'on_the_way' ? 'in_progress' : 'completed' }}')" 
                            class="mt-2 bg-green-600 text-white px-4 py-2 rounded-lg text-sm font-medium hover:bg-green-700 transition">
                            {{ $key == 'on_the_way' ? 'Sampai di Lokasi' : ($key == 'in_progress' ? 'Selesaikan Pekerjaan' : '') }}
                        </button>
                        @endif
                    </div>
                </div>
