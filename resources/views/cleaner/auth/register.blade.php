...
<div class="min-h-screen bg-gradient-to-br from-[#f0fdf5] to-[#d3fcf2] flex flex-col justify-center items-center p-6">
    ...
    <div class="mb-8 text-center pt-12">
        ...
        <h3 class="text-2xl font-bold text-black mb-1">Daftar Petugas Yuk Clean</h3>
        <p class="text-gray-600">Buat akun baru untuk mulai bekerja</p>
    </div>
    ...
    <div class="bg-white rounded-2xl shadow-xl p-8 w-full max-w-md border border-[#cfcfcf]">
        <form method="POST" action="{{ route('cleaner.register.submit') }}">
            ...
            <div class="mb-4">
                ...
                <select name="gender" 
                    class="w-full px-4 py-3 rounded-lg border bg-white {{ $errors->has('gender') ? 'border-red-500' : 'border-gray-200' }} focus:outline-none focus:ring-2 focus:ring-[#cfcfcf]" 
                    required>
    ...
    <p class="text-center mt-4 text-gray-600">
        Sudah punya akun? 
        <a href="{{ route('cleaner.login') }}" class="text-[#00bda2] font-medium hover:underline">
            Masuk
        </a>
    </p>
