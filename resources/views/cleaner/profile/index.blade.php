...
<div class="pb-24 bg-white">
    ...
    <div class="bg-linear-to-r from-green-500 to-green-600 p-6 text-white">
        <div class="flex items-center justify-between">
            ...
            <a href="{{ route('cleaner.profile.edit') }}" class="bg-white/20 p-2 rounded-full">
        ...
        <div class="bg-white rounded-2xl shadow-lg p-6 border border-gray-100">
            <div class="flex items-center gap-4">
                ...
                <div class="flex-1">
                    ...
                    <div class="flex items-center mt-1">
                        ...
                        <span class="font-medium">{{ $cleaner->formatted_rating }}</span>
                    </div>
                </div>
            </div>
            ...
            <div class="grid grid-cols-3 gap-3 mt-6">
                <div class="text-center">
                    <span class="block text-2xl font-bold text-green-600">{{ $cleaner->total_tasks }}+</span>
                    <span class="text-xs text-gray-500">Tugas Selesai</span>
                </div>
                <div class="text-center">
                    <span class="block text-2xl font-bold text-green-600">{{ $cleaner->formatted_rating }}</span>
                    <span class="text-xs text-gray-500">Rating</span>
                </div>
                <div class="text-center">
                    <span class="block text-2xl font-bold text-green-600">{{ $cleaner->satisfaction_rate }}%</span>
        ...
        <div class="bg-white rounded-xl border border-gray-200 p-5 shadow-sm">
            <div class="space-y-4">
                <div class="flex justify-between items-center">
                    ...
                    <span class="font-semibold">{{ $monthlyPerformance->tasks_completed ?? $cleaner->total_tasks }} tugas</span>
                </div>
                <div class="w-full bg-gray-200 rounded-full h-2">
                    <div class="bg-green-600 rounded-full h-2" style="width: {{ min(100, ($monthlyPerformance->tasks_completed ?? 24) * 4) }}%"></div>
                </div>

                <div class="flex justify-between items-center">
                    <span class="text-sm text-gray-600">Hari Aktif</span>
                    <span class="font-semibold">{{ $monthlyPerformance->active_days ?? $cleaner->active_days }} hari</span>
                </div>
                <div class="w-full bg-gray-200 rounded-full h-2">
                    <div class="bg-green-600 rounded-full h-2" style="width: {{ min(100, ($monthlyPerformance->active_days ?? 20) * 3.33) }}%"></div>
                </div>

                <div class="flex justify-between items-center">
                    <span class="text-sm text-gray-600">Rating Rata-rata</span>
                    <span class="font-semibold">{{ $monthlyPerformance->avg_rating ?? $cleaner->rating }}</span>
                </div>
                <div class="flex items-center">
                    @for($i = 1; $i <= 5; $i++)
                        <svg class="w-5 h-5 {{ $i <= ($monthlyPerformance->avg_rating ?? $cleaner->rating) ? 'text-yellow-400' : 'text-gray-300' }}" fill="currentColor" viewBox="0 0 20 20">
            ...
            <div class="bg-white rounded-xl border border-gray-200 p-4 shadow-sm">
                <div class="flex items-center gap-3">
                    ...
                    <div class="flex-1">
                        ...
                        <p class="text-xs text-gray-400">{{ $task->completed_at->format('d M Y') }}</p>
                    </div>
