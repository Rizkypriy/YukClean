...
<body class="bg-gray-50">
    ...
    {{-- Bottom Navigation (Hanya untuk halaman selain login/register) --}}
    @if(!request()->is('login') && !request()->is('register'))
    <nav class="fixed bottom-0 left-0 right-0 bg-white border-t border-gray-200 py-2 px-4 z-50">
        <div class="flex justify-around items-center">
            <a href="{{ route('home') }}" class="flex flex-col items-center {{ request()->routeIs('home') ? 'nav-active text-green-600' : 'text-gray-500' }}">
                <i class="fas fa-home text-xl"></i>
                <span class="text-xs mt-1">Home</span>
            </a>
            <a href="{{ route('orders.index') }}" class="flex flex-col items-center {{ request()->routeIs('orders.*') ? 'nav-active text-green-600' : 'text-gray-500' }}">
                <i class="fas fa-clipboard-list text-xl"></i>
                <span class="text-xs mt-1">Pesanan</span>
            </a>
            <a href="{{ route('promo.index') }}" class="flex flex-col items-center {{ request()->routeIs('promo.*') ? 'nav-active text-green-600' : 'text-gray-500' }}">
                <i class="fas fa-tags text-xl"></i>
                <span class="text-xs mt-1">Promo</span>
            </a>
            <a href="{{ route('profile.index') }}" class="flex flex-col items-center {{ request()->routeIs('profile.*') ? 'nav-active text-green-600' : 'text-gray-500' }}">
